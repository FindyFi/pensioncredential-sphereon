...
async function getOffer(path) {
  ...
  const createUrl = `${config.issuer_base}/webapp/credential-offers`
  const credentialParams = {
    "credential_configuration_ids": [ config.definitionId ],
    "grants": {
      "urn:ietf:params:oauth:grant-type:pre-authorized_code": {
        "pre-authorized_code": "using the same code for all requests..."
      }
    },
    "qrCodeOpts": {
      "size": 256
  ...
  if (!resp.ok) {
    ...
    throw new Error(JSON.stringify(await resp.text(), null, 2))
  }
...
<html lang="en">
 <meta charset="UTF-8">
 <title>Sphereon myöntää eläketodisteen</title>

...
async function createRequest() {
  ...
  const defUrl =  `${config.verifier_base}/oid4vp/webapp/definitions/${config.definitionId}/auth-requests`
  const resp = await fetch(defUrl, { method: 'POST', headers })
  console.error(resp.status, defUrl, headers)
  if (!resp.ok) {
    console.error(resp.status, defUrl, headers)
...
<html lang="en">
 <meta charset="UTF-8">
 <title>Sphereon vastaanottaa eläkeläistodisteen</title>
 <style>
   *[lang]:not([lang="en"]) {
   display: none;
  }
 ...
 <body style="text-align: center;">
  ...
  <div id="content">
   <a href="${request.authRequestURI}"><img src="${dataURL}" alt="Credential Request QR Code" /></a>
...
async function getStatus(correlationId, definitionId, includeVerifiedData='vp') {
  const headers = apiHeaders
  const statusUrl =  `${config.verifier_base}/oid4vp/webapp/auth-status`
  const params = {
    method: 'POST',
    headers,
    body: JSON.stringify({
  ...
  const resp = await fetch(statusUrl, params)
...
const handleRequests = async function (req, res) {
  ...
  switch (fullUrl.pathname) {
    case '/.well-known/openid-federation':
      ...
      return false
    case '/status':
      const includeVerifiedData = fullUrl.searchParams.get('includeVerifiedData')
      const correlationId = fullUrl.searchParams.get('correlationId')
      const definitionId = fullUrl.searchParams.get('definitionId')
      const status = await getStatus(correlationId, definitionId, includeVerifiedData)
